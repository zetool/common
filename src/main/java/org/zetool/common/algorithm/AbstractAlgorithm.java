 ...
 * @author Martin Groß
 */
public abstract class AbstractAlgorithm<P, S> implements Algorithm<P, S> {

    private Throwable cause;

    /* An enumeration type that specifies the current state of the algorithm. */
    public enum State {
        /** No problem has been set yet. */
        UNINITIALIZED,
        /** A problem is defined, but solving is not started. */
        WAITING,
    ...
    /**
     * Adds the specified listener to the set of listeners receiving events from this algorithm. If the specified
     * listener is already part of this list, nothing happens. When the problem is already solved, no more events will
    ...
    public final void addAlgorithmListener(AlgorithmListener listener) {
        if (isProblemSolved()) {
            throw new IllegalStateException("The problem has already been solved." + 
                    " There will be no more events that could be listened to anymore.");
        } else {
     ...
     */
    public final void addAlgorithmListener(AbstractAlgorithm<P, S> algorithm) {
        Set<AlgorithmListener> s = algorithm.algorithmListeners;
        s.stream().forEach(listener -> addAlgorithmListener(listener));
    }
    ...
    /**
     * Dispatches an algorithm progress event with the specified message and current progress value to all listeners.
     * The method is a shortcut for {@code fireEvent(String.format(formatStr, params))}.
     *
    ...
    /**
     * Checks if the progress is valid, i.e. is larger than the old progress and it is larger than the
     * {@link #accuracy}.
     *
     * @param progress the new progress
     * @return {@literal true} if an event for the given progress should be fired, {
     * @false otherwise}
     */
    ...
    public final void setProblem(P problem) {
        if (state == State.SOLVING) {
            throw new IllegalStateException("The algorithm is currently running!"
    ...
    /**
     * Returns the runtime of the algorithm as a string formatted with regard to human readability. The formatting is
     * done according to {@link Formatter#formatUnit(double, org.zetool.common.util.units.UnitScale, int)}.
     *
    ...
    /**
     * Sets the level from that messages are logged by the {@link Logger} of this algorithm. By default, messages
     * starting from {@link java.util.logging.Level#CONFIG} are given out using {@link System#out} and warnings and
     * errors are sent to {@link System#err}.
     *
    ...
    /**
     * Returns the logger currently used by the {@link AbstractAlgorithm}.
     *
     * @return the logger currently used by the {@link AbstractAlgorithm}
     */
    ...
    /**
     * Sets a specific logger for the {@link AbstractAlgorithm}.
     *
    ...
    public final void setLoggingEvents(boolean loggingEvents) {
        ...
        } else { // remove all existing loggers
            List<AlgorithmListener> logger = new LinkedList<>();
            algorithmListeners.stream().filter(listener -> listener instanceof AbstractAlgorithm.EventLogger).forEach(
                    listener -> logger.add(listener));
            algorithmListeners.removeAll(logger);
        }
    }

    /**
     * Returns whether a problem instance has been specified for the algorithm. This is the prerequisite for beginning
    ...
    /**
     * Returns whether this algorithm has successfully run and solved the instance of the problem given to it. If this
     * is {@code true}, then the solution to the instance of the problem can be obtained by {@code
     * getSolution}.
     *
    ...
    public final void runAlgorithm() {
        if (!isProblemInitialized()) {
            throw new IllegalStateException("The instance of the problem has been specified yet." +
        ...
        public void eventOccurred(AbstractAlgorithmEvent event) {
            ...
            if (event instanceof AlgorithmStartedEvent) {
                message = String.format("%1$s: %2$s gestartet...",
                        event.getFormattedEventTime(), AbstractAlgorithm.this.getClass().getSimpleName());
                ...
                message = String.format("%1$s: %2$s beendet nach %3$s.",
                        event.getFormattedEventTime(), AbstractAlgorithm.this.getClass().getSimpleName(), getRuntimeAsString());
            }
            if (!message.isEmpty()) {
                log(message);
            }
