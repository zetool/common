...
public class LocalizationManager {
    ...
    private static volatile LocalizationManager singleton;
    /** Mapping of property files to localizations. */
    private final Map<String, AbstractLocalization> localization = new HashMap<>();
    ...
    /**
     * Loads a new localization resource file from hard disk and sets new localized number converters. The language is
     * set by a {@link java.util.Locale} object.
     *
     * <p>
     * The localization file has to be found in the localization folder and has the name zevacuate.properties with the
     * language information respectively.
     *
    ...
    public final void setLocale(Locale locale) {
        ...
        nfInteger = NumberFormat.getIntegerInstance(currentLocale);
        nfPercent = NumberFormat.getPercentInstance(currentLocale);
        localization.values().stream().forEach(loc -> loc.setLocale(locale));
    }

    /**
     * Returns the currently selected {@link java.util.Locale}, that allows to format and read localized numbers.
     *
